...
function usuarioEhAdmin(mysqli $conn): bool
{
    if (!usuarioLogado()) {
        return false;
    }
    ...
    $stmt = $conn->prepare('SELECT is_admin FROM usuarios WHERE id = ? AND ativo = 1 LIMIT 1');
    if (!$stmt) {
        return false;
    }
    ...
    return !empty($row) && (int)($row['is_admin'] ?? 0) === 1;
}

function exigirAdmin(mysqli $conn, string $destino = '../entrar.php'): void
{
    exigirLogin($destino);

    if (!usuarioEhAdmin($conn)) {
