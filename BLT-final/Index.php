    ...
    <nav class="bl-nav" aria-label="Navegação principal">
      ...
      <?php if ($logado): ?>
        <a class="perfil-link" href="php/perfil.php">
          <span class="avatar"><?= e(mb_strtoupper(mb_substr($nome, 0, 1))) ?></span>
          <span><?= e($nome) ?></span>
        </a>
      <?php else: ?>
...
<section class="bl-section" id="jogos">
  <div class="container">
    ...
    <div class="bl-cards">
      <article class="bl-card bl-game-card jogo-card <?= $logado ? '' : 'is-locked' ?> bl-reveal">
        <div class="bl-badges">
        ...
        <?php if (!$logado): ?>
        <div class="bl-locked-overlay"><i class="fa-solid fa-lock"></i><span>Requer conta e assinatura</span></div>
        <?php endif; ?>
      </article>
...
function pesquisar(){
  ...
  document.querySelectorAll('.jogo-card').forEach(card=>{
    card.style.display=(!termo||card.innerText.toLowerCase().includes(termo))?'':'none';
  });
